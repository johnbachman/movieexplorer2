...
public class Stack_Splitter_2 implements PlugIn {
    ...
    protected void stack2stacks(ImagePlus imp,
        ...
        for(int nss=1; nss<=numSubStacks; ++nss) {
            ...
            // Populate this stack.
            for(int n=1; n<=numImages; ++n) {      // RWC
                imp.setSlice((n-1)*numSubStacks+nss);  // activate next slice *** RWC
                
                // RWC: Create a label, using the slice label if possible

                    sSliceLabel = "channel_"+df.format(nss)+"_slice_"+df.format(n);

                // Get pixels from associated processor. Make sure
                // input pixels are copy of original.
                // ImageProcessor.crop() could also be used here.
                ims.addSlice(sSliceLabel,            // RWC
                             imp.getProcessor().getPixelsCopy()); // ***
            }
